...
interface FrameData {
  ...
  windows: WindowInfo[];
  animating: boolean;       // True during any activity (zoom/pan/drag) - for framerate
  transitioning: boolean;   // True only during viewport animations (workspace switches)
        ...
        const render = (time: number) => {
          ...
          if (time - lastTime >= interval) {
            ...
            try {
              ...
              // Show all workspaces when in void OR transitioning between workspaces
              // "transitioning" is true only during viewport animations (workspace switches)
              // NOT during user zoom/pan - those should stay within current workspace
              ...
              for (const [id, el] of windowRefsMap.current.entries()) {
                if (!currentWindowIds.has(id)) {
                  // Window was filtered out - hide it immediately
