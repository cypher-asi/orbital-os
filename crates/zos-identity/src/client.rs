...
impl IdentityClient {
    ...
    /// 4. Encrypt 2 shards with the password and store to keystore
    /// 5. Store public keys to VFS
    /// 6. Return 3 paper shards + public identifiers
    ///
    ...
    /// - `password`: Password to encrypt 2 stored shards (minimum 12 characters)
    ///
    /// # Returns
    /// - `Ok(NeuralKeyGenerated)` with paper shards and public identifiers
    /// - `Err(KeyError)` on failure
    pub fn generate_neural_key(
        &self,
        user_id: UserId,
        password: String,
    ) -> Result<NeuralKeyGenerated, KeyError> {
        let request = GenerateNeuralKeyRequest { user_id, password };
    ...
    /// - `key_scheme`: Key scheme to use for the machine key
    /// - `external_shard`: Single external Neural shard (from paper backup)
    /// - `password`: Password to decrypt stored shards
    ///
    /// # Returns
