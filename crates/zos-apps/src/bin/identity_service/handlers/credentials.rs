...
pub fn handle_attach_email(service: &mut IdentityService, msg: &Message) -> Result<(), AppError> {
    ...
    service.start_network_fetch(&http_request, zos_apps::identity::pending::PendingNetworkOp::SubmitEmailToZid {
...
pub fn continue_attach_email_after_zid(service: &mut IdentityService, client_pid: u32, user_id: u128, email: String, cap_slots: Vec<u32>) -> Result<(), AppError> {
    let cred_path = CredentialStore::storage_path(user_id);
    service.start_storage_read(&format!("content:{}", cred_path), PendingStorageOp::ReadCredentialsForAttach { client_pid, user_id, email, cap_slots })
}

pub fn continue_attach_email_after_read(service: &mut IdentityService, client_pid: u32, user_id: u128, email: String, existing_store: Option<CredentialStore>, cap_slots: Vec<u32>) -> Result<(), AppError> {
    ...
    match serde_json::to_vec(&store) {
        Ok(json_bytes) => service.start_storage_write(&format!("content:{}", cred_path), &json_bytes.clone(), PendingStorageOp::WriteEmailCredentialContent { client_pid, user_id, json_bytes, cap_slots }),
...
pub fn handle_get_credentials(service: &mut IdentityService, msg: &Message) -> Result<(), AppError> {
    ...
    service.start_storage_read(&format!("content:{}", cred_path), PendingStorageOp::GetCredentials {
        client_pid: msg.from_pid,
        cap_slots: msg.cap_slots.clone(),
    })
}
...
pub fn handle_unlink_credential(service: &mut IdentityService, msg: &Message) -> Result<(), AppError> {
    ...
    let cred_path = CredentialStore::storage_path(request.user_id);
    service.start_storage_read(&format!("content:{}", cred_path), PendingStorageOp::ReadCredentialsForUnlink {
        client_pid: msg.from_pid,
        user_id: request.user_id,
...
pub fn continue_unlink_credential_after_read(service: &mut IdentityService, client_pid: u32, user_id: u128, credential_type: CredentialType, data: &[u8], cap_slots: Vec<u32>) -> Result<(), AppError> {
    ...
    let cred_path = CredentialStore::storage_path(user_id);
    match serde_json::to_vec(&store) {
        Ok(json_bytes) => service.start_storage_write(&format!("content:{}", cred_path), &json_bytes.clone(), PendingStorageOp::WriteUnlinkedCredentialContent { client_pid, user_id, json_bytes, cap_slots }),
