...
This document defines the acceptance criteria and test cases for `orbital-desktop`. All tests must pass before the crate is considered complete.
...
# CI script to verify
#!/bin/bash
for file in $(find crates/orbital-desktop/src -name "*.rs"); do
    awk '
...
fn count_mount_elements() -> usize {
    // Count elements with class "orbital-mount" in DOM
}
```
